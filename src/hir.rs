...
use nom::{
	branch::alt,
	bytes::complete::{escaped, tag, take_while, take},
...
pub fn parse_hir<'a>(i: &'a str) -> Result<Expr, nom::Err<VerboseError<&'a str>>> {
	...
	if i.len() > 0 {
		Err(nom::Err::Failure(VerboseError::from_error_kind(i, nom::error::ErrorKind::Complete)))
	} else {
		Ok(expr)
	}
}
...
fn biguint<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, BigUint, E> {
	let (i, _) = ws(i)?;
	map(digit1, |s| Num::from_str_radix(s, 10).expect("nom::digit matched a non-int?"))(i)
}
...
fn ident<'a, E: nom::error::ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, &'a str, E> {
	...
	// check first char
	{
		...
		if first_char != '_' && !first_char.is_alphanumeric() {
			return Err(Err::Error(E::from_error_kind(i, nom::error::ErrorKind::AlphaNumeric)))
		}
	}
